...
class EpuckBasic (DifferentialWheels):
	...
	def basic_setup(self, tempo = 1.0):
		...
		self.leds = [self.getLED('led'+str(x)) for x in range(9)] #led
		map((lambda s: s.set(0)), self.leds)
	...
	def get_lights(self):
		...
		self.light_sensor_values = [self.light_sensor_values[x] for x in lights_order]
